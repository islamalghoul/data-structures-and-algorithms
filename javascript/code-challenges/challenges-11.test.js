...
function transformToLis(obj){
  ...
  let arrayOfValurs=Object.values(obj)
  let array= []
  for (let i = 0; i < arrayOfKeys.length; i++) {
    array.push(`<li>${arrayOfKeys[i]}: ${arrayOfValurs[i]}</li>`)
...
let findShortest = (data) => {
  ...
  let name=''
  data.forEach(ele=>{
if(parseInt(ele.height)<min){
  min=parseInt(ele.height)
}
  })
  data.forEach(e=>{
    if(parseInt(e.height)==min){
       name=e.name
    }
  })
  return name
