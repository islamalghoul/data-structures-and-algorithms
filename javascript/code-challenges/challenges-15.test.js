...
const screenForNames = (arr) => {
  ...
  return arr.filter(ele => {
    ...
    return /^(\bMr. \w+)$|^(\bMrs. \w+)$|^(\bMs. \w+)$|^(\bDr. \w+)$/g.test(ele)
  })
}
...
const toTitleCase = (arr) => {
  // Solution code here...
  return arr.map(ele => {
    return ele = ele[0].toUpperCase() + ele.slice(1)
  })
};
...
const isSecure = (url) => {
  // Solution code here...
  return /^(https:\/\/\w+.\w+)$|^(\bhttps:\/\/\w+.\w+.\w+)$/.test(url)
...
const detectTicTacToeWin = (board) => {
  ...
  for (let i = 0; i <3 ; i++) {
    ...
    break;
    for (let j = 0; j < 3 ; j++) {
      if (board[0][j] == board[1][j] && board[1][j]== board[2][j]&&board[2][j]== 'X'||board[0][j] == board[1][j] &&board[1][j]== board[2][j]&& board[2][j]== 'O') {
        winOrLose = true
        break;
      }
      else if (board[i][0] == board[i][1] &&board[i][1]== board[i][2]&&board[i][2]== 'X'||board[i][0] == board[i][1] &&board[i][1]== board[i][2]&&board[i][2]== 'O') {
        winOrLose = true
        break;
      }
      else if(board[0][0] == board[1][1]&& board[1][1]== board[2][2]&&board[2][2] == 'X' || board[0][0] == board[1][1]&& board[1][1]== board[2][2]&&board[2][2] == 'O'){
        winOrLose = true
      break;
    }
        else if(board[0][2] == board[1][1]&&board[1][1] == board[2][0]&&board[2][0]== 'X' ||board[0][2] == board[1][1]&&board[1][1] == board[2][0]&&board[2][0]== 'O')
       { 
        winOrLose = true
        break;
      }
